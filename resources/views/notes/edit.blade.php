<x-layouts.app>
    ...
    <form id="book" action="/notes" method="post" class="md:w-1/2 w-[90%]  my-10 m-auto border border-orange-500 rounded">
        ...
        <div class="flex   items-center gap-2 p-5 ">
            <input type="text" name="title" value="{{$note->title}}" placeholder="title" class="bg-white grow border-b-2 text-center outline-0 p-4">
            <input type="hidden" id="color" name="color" value="{{$note->color}}">
        ...
        <div id="note-book" class="p-4">
            @for($i=0;$i< 20; $i++)
                @if ($i < count($lines))
                @if ($lines[$i] !=null)
                <input type="text" name="input{{$i}}" value="{{$lines[$i]}}" class="pt-2 outline-0 border-b block w-full">
                @endif
                @else
                <input type="text" name="input{{$i}}" value="" class="pt-2 outline-0 border-b block w-full">
                @endif
                @endfor
        </div>
        <button class="w-full bg-orange-500 text-white font-bold p-3">Update</button>
        <script>
